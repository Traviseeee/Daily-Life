const Login = {
  ...
  open(mode = "login") {
    ...
    const hasAccount = Boolean(appData.profile.name && appData.profile.passwordHash);
    ...
    modal.innerHTML = `
      <div class="login-content">
        <div class="login-header">
          ...
          <h2>${isReset ? t("resetPassword") : hasAccount ? t("unlockMylife") : t("createAccount")}</h2>
          <p>${isReset ? t("resetPasswordHelp") : hasAccount ? t("enterPassword") : t("createAccountHelp")}</p>
        </div>
        <form id="loginForm">
          ...
          ${isReset ? "" : `<div class="login-input-group">`}
          ...
          <button type="submit" class="button login-button">${isReset ? t("saveNewPassword") : hasAccount ? t("unlock") : t("createAccount")}</button>
    ...
    document.getElementById("loginForm").addEventListener("submit", (e) => {
      e.preventDefault();
      this.submit(hasAccount, isReset);
    });
  ...
  async submit(hasAccount, isReset) {
    ...
    if (!hasAccount && !name) return showError(t("nameRequired"));
    ...
    if (!hasAccount) Store.updateProfile({ name, passwordHash: hash, recoveryAnswerHash: await this.hash(recoveryAnswer.toLowerCase()) });
